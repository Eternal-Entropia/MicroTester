// ============================================================
// [Signal Generator] — вкладка "Signal Generator" (панель #panel-pwmdac)
// 2nd-Order Sigma-Delta DAC waveform output on PB5 @ 42 MHz SPI+DMA
// (sine / saw / triangle / square / DC / noise).
// (ранее файл назывался pwmdac.js — переименован для ясности)
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Noise random helpers (deterministic per index for reproducible bitstream) ---
    function sdPrng(n) {
        let x = (Math.imul(n, 2654435761) ^ 0x9E3779B9) >>> 0;
        x ^= x >>> 13; x = Math.imul(x, 0x5bd1e995); x ^= x >>> 15;
        return (x >>> 0) / 4294967296; // [0,1)
    }
    function sdNoiseStepIndex(i, freqHz) {
        const hold = Math.max(1, Math.round(42000000 / Math.max(1, freqHz)));
        return Math.floor(i / hold);
    }
    function sdNoiseVal(n) {
    ...
    function pickSdParams(freqHz) {
        ...
        for (let e = 0; e <= 7; e++) {
            ...
            if (br <= MAX_BITS * Math.max(1, freqHz)) { exp = e; break; }
        }
        const bitRate = 84000000 / Math.pow(2, exp + 1);
        const minFreq = bitRate / MAX_BITS;
        const actual = (freqHz < minFreq) ? minFreq : freqHz;
        return { prescalerExp: exp, bitRate, minFreq, actual };
    }
    ...
    function generateSigmaDeltaBitstream(mode, freqHz, dutyPct = 50) {
        ...
        for (let i = 0; i < totalBits; i++) {
            ...
            switch (mode) {
                ...
                case 'sd_noise': normVal = sdNoiseVal(sdNoiseStepIndex(i, freqHz)) * 0.6; break;
        ...
        return { buffer, totalBits, numBytes, actualFreq, prescalerExp, bitRate };
    }
    ...
    function updateCalculationsAndPreview() {
        ...
        let freq = Math.max(1, Math.min(1000000, parseInt(cfgPwmDacFreq?.value || '1000')));
        const isNoise = (mode === 'sd_noise');
        if (isNoise && freq < 100000) {
            freq = 100000;
            if (cfgPwmDacFreq) cfgPwmDacFreq.value = freq;
            if (cfgPwmDacFreqRange) cfgPwmDacFreqRange.value = Math.log10(freq);
        }
        ...
        const isDutyDisabled = (mode === '0' || mode === 'sd_sine' || mode === 'sd_saw' || mode === 'sd_tri' || isNoise);
        ...
        if (isDc) {
            ...
            const osr = Math.max(1, Math.round(sdBitRate / Math.max(1, sdActualFreq)));
        ...
        if (canvas) {
            ...
            // Hi-DPI rendering: sharp text/lines on any display
            ...
            else if (mode === 'sd_noise') waveformName = 'WHITE NOISE (UNIFORM)';
            ...
            for (let i = 0; i <= points; i++) {
                ...
                else if (mode === 'sd_noise') val = sdNoiseVal(i) * 0.6;
            ...
            const { bitRate, actual: actualFreq } = pickSdParams(freq);
            const osr = Math.max(1, Math.round(bitRate / Math.max(1, actualFreq)));
            let enob = Math.min(16, Math.max(1, Math.round(2.5 * Math.log2(osr) - 1.5)));
    ...
    async function sendPwmDacStart() {
        ...
        if (isSigmaDelta) {
            ...
            if (isNoise && freq < 100000) freq = 100000;
            const duty = parseInt(cfgPwmDacDuty?.value || '50');
            const { buffer, numBytes, prescalerExp, actualFreq } = generateSigmaDeltaBitstream(mode, freq, duty);
            ...
            const startPayload = new Uint8Array(4 + firstChunkLen);
            startPayload[0] = pinVal;
            startPayload[1] = prescalerExp;
            startPayload[2] = numBytes & 0xFF;
            startPayload[3] = (numBytes >> 8) & 0xFF;
            startPayload.set(buffer.subarray(0, firstChunkLen), 4);
